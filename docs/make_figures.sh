...
# docs/tests/dag.png -- Snakemake job DAG for a two-sample run, from the
#                       workflow itself, so it cannot describe a pipeline that
#                       no longer exists. It is built against empty stub Cell
#                       Ranger directories: --dag only needs the input paths to
#                       exist, not to contain anything.
#
