...
from three skip flags to one (`doublet.skip`). In v0.1.0 the "last enabled stage must emit `result/{s}`"
...
(scatter, hexbin) with `rasterized=True` at `dpi=500`; a PNG twin at 200 dpi for the HTML report only.
