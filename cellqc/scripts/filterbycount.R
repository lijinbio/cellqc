...
ggsave(p, file=sprintf('%s/feature_bf.png', outdir), width=9, height=7.5, units='in', dpi=200)
...
p=VlnPlot(x, features=c('nCount_RNA', 'nFeature_RNA', 'percent.mt'), ncol=3, log=F, pt.size=0) + NoLegend()
ggsave(p, file=sprintf('%s/feature_af.png', outdir), width=9, height=7.5, units='in', dpi=200)
